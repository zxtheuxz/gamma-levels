...
type StudyTicker = "PETR4" | "VALE3" | "ITUB4" | "BBDC4";
const studyTickers: StudyTicker[] = ["PETR4", "VALE3", "ITUB4", "BBDC4"];
...
export default function Home() {
  ...
  const loadSecondary = useCallback(async () => {
    const [historyResponse, qualityResponse, studyResponse, studyStatusResponse] = await Promise.all([
      ...
      <section className="workspace">
        <header className="topbar">
          ...
          <div className="header-actions">
            ...
            {tab === "history" && <button className="ghost-button" onClick={() => setStudyTicker(studyTickers[(studyTickers.indexOf(studyTicker) + 1) % studyTickers.length])}>Exibindo {studyTicker}</button>}
...
function StudyHistory({ study, legacy, onRun, running, ticker }: { study: StudyPayload; legacy: HistoryPayload; onRun: () => void; running: boolean; ticker: string }) {
  ...
  const primary = study.metrics.filter((item) => item.variant === "region_v1" && item.sample === "full" && item.overlap_mode === "INDEPENDENT");
  ...
  const annualTrades = study.trades.filter((row) => row.variant === "baseline_v0" && row.strategy === "HOLD_TO_EXPIRY" && row.overlap_mode === "INDEPENDENT");
  ...
  return (
    <>
      <div className="history-stats">
        ...
        <article><span>OPERAÇÕES</span><strong>{hold?.trades ?? legacy.signals}</strong></article>
      ...
      {primary.length ? (
        <>
          <div className="section-heading"><div><p className="eyebrow">SEIS REGRAS</p><h2>Comparação de saídas</h2></div><p>Expectativa líquida é a métrica principal; custos começam em zero e permanecem configuráveis.</p></div>
